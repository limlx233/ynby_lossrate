...
with st.container(border=True):
    ...
    if uploaded_file1 is not None and uploaded_file2  is not None :
        ...
        def create_excel():
            ...
            # 写入异常数据
            if st.session_state.fhg_outlier_p5 is not None and not st.session_state.fhg_outlier_p5.empty:
                st.session_state.fhg_outlier_p5.to_excel(writer, sheet_name='复合管异常数据', index=False)
            if st.session_state.zh_outlier_p5 is not None and not st.session_state.zh_outlier_p5.empty:
                st.session_state.zh_outlier_p5.to_excel(writer, sheet_name='纸盒异常数据', index=False)
            if st.session_state.zx_outlier_p5 is not None and not st.session_state.zx_outlier_p5.empty:
                st.session_state.zx_outlier_p5.to_excel(writer, sheet_name='纸箱异常数据', index=False)
            # 写入负损耗数据
            if st.session_state.fhg_low_loss_rate is not None and not st.session_state.fhg_low_loss_rate.empty:
                st.session_state.fhg_low_loss_rate.to_excel(writer, sheet_name='复合管负损耗数据', index=False)
            if st.session_state.zh_low_loss_rate is not None and not st.session_state.zh_low_loss_rate.empty:
                st.session_state.zh_low_loss_rate.to_excel(writer, sheet_name='纸盒负损耗数据', index=False)
            if st.session_state.zx_low_loss_rate is not None and not st.session_state.zx_low_loss_rate.empty:
                st.session_state.zx_low_loss_rate.to_excel(writer, sheet_name='纸箱负损耗数据', index=False)

            # 写入合并后的数据
            if st.session_state.fhg_p5 is not None:
                st.session_state.fhg_p5.to_excel(writer, sheet_name='复合管', index=False)
            if st.session_state.zh_p5 is not None:
                st.session_state.zh_p5.to_excel(writer, sheet_name='纸盒', index=False)
            if st.session_state.zx_p5 is not None:
                st.session_state.zx_p5.to_excel(writer, sheet_name='纸箱', index=False)
            
            # 写入IMR控制图参数
            if st.session_state.IMR_params_fhg_p5 is not None and not st.session_state.IMR_params_fhg_p5.empty:
                st.session_state.IMR_params_fhg_p5.to_excel(writer, sheet_name='IMR参数_复合管', index=False)
            if st.session_state.IMR_params_zh_p5 is not None and not st.session_state.IMR_params_zh_p5.empty:
                st.session_state.IMR_params_zh_p5.to_excel(writer, sheet_name='IMR参数_纸盒', index=False)
            if st.session_state.IMR_params_zx_p5 is not None and not st.session_state.IMR_params_zx_p5.empty:
                st.session_state.IMR_params_zx_p5.to_excel(writer, sheet_name='IMR参数_纸箱', index=False)
